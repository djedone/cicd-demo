...
def create_app(testing=False):
    ...
    def deployments():
        ...
        try:
            ...
            logs = (
                DeploymentLog.query.filter_by(environment=current_env)
    ...
    def stats():
        ...
        try:
            ...
            # Filter by environment
            current_env = os.getenv("ENVIRONMENT", "development")
            total_deployments = DeploymentLog.query.filter_by(
                environment=current_env
            ).count()
            successful_deployments = DeploymentLog.query.filter_by(
                environment=current_env, status="success"
            ).count()
            ...
            return jsonify(
                {
                    ...
                    "last_deployment": DeploymentLog.query.order_by(
                        DeploymentLog.deployed_at.desc()
                    )
                    .first()
    ...
    def custom_metrics():
        ...
        try:
            ...
            successful_recent = len(
                [d for d in recent_deployments if d.status == "success"]
            )
            ...
            env_stats = (
                ...
                .group_by(DeploymentLog.environment)
                .all()
            )
            ...
            for env, count in env_stats:
                ...
                env_data.append(
                    {"environment": env, "total": count, "successful": successful}
                )
            ...
            daily_stats = (
                ...
                .group_by(func.date(DeploymentLog.deployed_at))
                .all()
            )
            ...
            return jsonify(
                {
                    "summary": {
                        ...
                        "success_rate_30d": (successful_recent / total_recent * 100)
                        if total_recent > 0
                        else 0,
                    ...
                    "by_environment": [
                        {
                            ...
                            "success_rate": (env["successful"] / env["total"] * 100)
                            if env["total"] > 0
                            else 0,
                        }
                        for env in env_data
            ...
            return jsonify(
                {"error": str(e), "message": "Failed to generate custom metrics"}
            )

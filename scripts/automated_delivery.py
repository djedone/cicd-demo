...
class AutomatedDelivery:
    def __init__(self):
        self.staging_url = os.getenv('STAGING_URL', 'https://cicd-demo-staging-1s8u.onrender.com')
        self.production_url = os.getenv('PRODUCTION_URL', 'https://cicd-demo-e71o.onrender.com')
    ...
    def create_deployment_record(self, url, version, environment):
        ...
        try:
            response = requests.post(f"{url}/api/deployments", json=deployment_data, timeout=10)
            if response.status_code == 201:
                result = response.json()
                print(f"✅ Deployment recorded: {result['version']} to {result['environment']}")
                return True
            else:
    ...
    def create_github_deployment(self, environment, sha, ref):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                status_response = requests.post(f"{deployment_url}/statuses", json=status_data, headers=headers)
    ...
    def deliver_to_production(self):
        ...
        if not self.create_deployment_record(self.production_url, version, "production"):
            return False
    ...
    def rollback_deployment(self, environment):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                response = requests.post(f"{url}/api/deployments", json=rollback_data, timeout=10)
                if response.status_code == 201:

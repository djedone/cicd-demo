...
def test_database_connection():
    ...
    try:
        ...
        safe_url = (
            database_url.split("@")[0].split(":")[-1][:4]
            + "***"
            + "@"
        ...
        if table_exists:
            ...
            if count > 0:
                cursor.execute(
                    "SELECT version, environment, status, deployed_at FROM deployment_log ORDER BY deployed_at DESC LIMIT 3;"
                )
                records = cursor.fetchall()
                print("Recent deployments:")
                for record in records:
                    print(
                        f"  - {record[0]} to {record[1]} ({record[2]}) at {record[3]}"
                    )
        else:
            print(
                "INFO: deployment_log table does not exist - will be created on first app run"
            )

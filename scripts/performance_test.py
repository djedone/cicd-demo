...
class PerformanceTester:
    ...
    def benchmark_endpoint(
        ...
        for i in range(iterations):
            response_time, status_code = self.single_request_test(
        ...
        results = {
            ...
            "success_rate": len([c for c in status_codes if 200 <= c < 300])
            / len(status_codes),
        }

        print(
            f"  Results: Mean={results['response_times']['mean']:.3f}s, "
    ...
    def load_test(
        ...
        print(
            f"Load testing {method} {endpoint} with {concurrent_users} users for {duration}s..."
        )
        ...
        if results["total_requests"] > 0:
            ...
            results["requests_per_second"] = (
                results["total_requests"] / results["total_duration"]
            )
            ...
            results["success_rate"] = len(
                [c for c in status_codes if 200 <= c < 300]
            ) / len(status_codes)
        ...
        print(
            f"  Completed: {results['total_requests']} requests in {results['total_duration']:.1f}s"
        )
        print(
            f"  RPS: {results.get('requests_per_second', 0):.1f}, "
...
def main():
    ...
    parser.add_argument(
        "--output", default="performance_results.json", help="Output file"
    )
    ...
    parser.add_argument(
        "--duration", type=int, default=30, help="Duration for load test (seconds)"
    )
    ...
    try:
        ...
        if status_code != 200:
            ...
            sys.exit(1)
        print(
            f"Server is running at {args.url} (response time: {response_time:.3f}s)\n"
        )

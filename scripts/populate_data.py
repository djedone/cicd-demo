...
def create_sample_deployment(base_url, environment):
    ...
    deployment_data = {"version": random.choice(versions), "environment": environment}

    try:
        response = requests.post(
            f"{base_url}/api/deployments", json=deployment_data, timeout=10
        )
        if response.status_code == 201:
            result = response.json()
            print(
                f"✅ Created deployment: {result['version']} to {result['environment']}"
            )
            return True
        else:
            print(
...
def populate_environment(base_url, environment, count=10):
    ...
    print(
        f"✅ Successfully created {success_count}/{count} deployments for {environment}"
    )
